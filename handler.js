...
const forceJID = (message) => {
    if (!message) return message
    
    if (message.sender && message.sender.includes('@lid')) {
        if (message.key && message.key.participant && !message.key.participant.includes('@lid')) {
            message.sender = message.key.participant;
        } else if (message.key && message.key.remoteJid && !message.key.remoteJid.includes('@lid')) {
            message.sender = message.key.remoteJid;
        }
    }
    
    if (message.chat && message.chat.includes('@lid')) {
        if (message.key && message.key.remoteJid && !message.key.remoteJid.includes('@lid')) {
...
const groupMetadata = m.isGroup ? { ...(conn.chats[m.chat]?.metadata || await this.groupMetadata(m.chat).catch(_ => null) || {}), ...(((conn.chats[m.chat]?.metadata || await this.groupMetadata(m.chat).catch(_ => null) || {}).participants) && { participants: ((conn.chats[m.chat]?.metadata || await this.groupMetadata(m.chat).catch(_ => null) || {}).participants || []).map(p => ({ ...p, id: p.jid, jid: p.jid })) }) } : {}
const participants = ((m.isGroup ? groupMetadata.participants : []) || []).map(participant => ({ id: participant.jid, jid: participant.jid, admin: participant.admin }))
